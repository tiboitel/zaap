...
class RateLimiter:
    ...
    def __init__(self, ip_limit: int, ip_window: int, account_limit: int, account_window: int):
    ...
    def check(self, ip: str, account_id: str | None = None) -> int:
        ...
        with self._lock:
            ...
            if len(self._ip_timestamps[ip]) >= self.ip_limit:
                retry_after = self._get_retry_after(self._ip_timestamps[ip], self.ip_window)
                raise RateLimitError(retry_after)
            self._ip_timestamps[ip].append(now)

            if account_id:
                self._clean_old(self._account_timestamps[account_id], self.account_window)
